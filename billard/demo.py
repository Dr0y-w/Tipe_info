...
class mainWindow(QDialog):
    def __init__(self):
        ...
        self.i = 0
        #objet et space
        self.b1 = Ball([-10,0],2,10,(255,0,0),[0.1,0],1)
        #wall
        self.w1 = Wall([20,80],[20,-80])


        self.s = Space([self.b1],[self.w1])
...
#j'aime max3nce
    ...
    def paintGL(self):  # réécriture de la fonction de base pour afficher la scène                # appelé a chaque 'update' lancé par le timer
        ...
        # affichage des trait de collision
        liste = separation_en_taille_demo([self.s.walls[0].p1,self.s.walls[0].p2],0.3)
        glBegin(GL_LINES)
        for e in liste:
            d = distance(self.s.balls[0].centre,e)
            c = (1/(d*10+0.000001))*100
            glColor((c,c,c))
            glVertex2fv(self.s.balls[0].centre)
            glVertex2fv(e)
        glEnd()
